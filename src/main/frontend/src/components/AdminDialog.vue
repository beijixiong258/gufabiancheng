<template>
  <el-dialog v-model="visible" title="用户管理" width="860px">
    ...
    <div class="admin-table-view">
      <el-table v-loading="loading" :data="users" size="small" border table-layout="fixed">
        <el-table-column prop="zhanghao" label="账号" width="145" show-overflow-tooltip />
    ...
    <div v-loading="loading" class="admin-mobile-list">
      <article v-for="user in users" :key="user.id" class="admin-user-card">
        <div class="admin-user-head">
          <strong>{{ user.zhanghao }}</strong>
          <el-tag size="small" :type="user.quanxian === 'ADMIN' ? 'danger' : 'primary'">
            {{ user.quanxian === 'ADMIN' ? '管理员' : '普通用户' }}
          </el-tag>
        </div>
        <dl>
          <div><dt>手机号</dt><dd>{{ user.dianhua || '未填写' }}</dd></div>
          <div><dt>邮箱</dt><dd>{{ user.youxiang || '未填写' }}</dd></div>
        </dl>
        <div class="admin-user-actions">
          <el-button size="small" @click="openEdit(user.id)">编辑</el-button>
          <el-button size="small" type="danger" plain @click="handleDelete(user.id)">删除</el-button>
        </div>
      </article>
    </div>
...
async function submit() {
  ...
  if (editingId.value && form.mima && (form.mima.length < 6 || form.mima.length > 32)) {
    ElMessage.warning('密码应为6-32位')
    return
  }
  saving.value = true
  try {
    if (editingId.value) {
      ...
      if (form.mima.length < 6 || form.mima.length > 32) {
        ElMessage.warning('密码应为6-32位')
        saving.value = false
        return
      }
...
.admin-form-actions :deep(.el-button + .el-button),
